...
public class QuotedStringTokenizer extends StringTokenizer {
	...
	public QuotedStringTokenizer(String str, String delim,
		...
		_token = new StringBuilder(_string.length() > 1024 ? 512
				: _string.length() / 2);
	}
	...
	/**
	 * Quote a string into a StringBuilder. The characters ", \, \n, \r, \t, \f,
	 * \b are escaped. Quotes are forced if any escaped characters are present
	 * or there is a ", ', space, +, =, ; or % character.
	 * 
	 * @param buf
	 *            The StringBuilder
	 * @param s
		...
		for (int i = 1; i < s.length() - 1; i++) {
			...
			if (escape) {
				...
				case 'b':
					...
					break;
				case 'u':
					b.append((char) ((TypeUtil.convertHexDigit((byte) s
							.charAt(i++)) << 24)
							+ (TypeUtil.convertHexDigit((byte) s.charAt(i++)) << 16)
							+ (TypeUtil.convertHexDigit((byte) s.charAt(i++)) << 8) + (TypeUtil
