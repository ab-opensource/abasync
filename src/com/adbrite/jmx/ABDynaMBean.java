...
public class ABDynaMBean implements DynamicMBean {
	...
	private static final Logger log = LoggerFactory.getLogger(ABDynaMBean.class);
    ...
    public ABDynaMBean(Object instance) {
        if(instance == null)
    ...
    private void findDescription() {
        ...
        if(mbean != null && mbean.description() != null && mbean.description().trim().length() > 0) {
            ...
            MBeanAttributeInfo info=new MBeanAttributeInfo(ABDynaMBean.MBEAN_DESCRITION,
                                                           "java.lang.String",
                                                           "MBean description",
                                                           true,
                                                           false,
                                                           false);
            try {
                atts.put(ABDynaMBean.MBEAN_DESCRITION,
    ...
    public synchronized MBeanInfo getMBeanInfo() {
    	String name = getObject().getClass().getCanonicalName();
    	if(name==null)
    		name=getObject().getClass().getSuperclass().getCanonicalName();
        return new MBeanInfo(name,
                             description,
                             attrInfo,
                             null,
                             opInfo,
    ...
    private void findMethods() {
        ...
        for(Method method:methods) {
            ...
            if(method.isAnnotationPresent(ManagedAttribute.class)) {
                ...
                String descr=attr != null ? attr.description() : null;
                exposeManagedAttribute(method, attributeName, descr);
            }
            //does method have spring framework @ManagedAttribute annotation?
            else if(method.isAnnotationPresent(org.springframework.jmx.export.annotation.ManagedAttribute.class)) {
            	org.springframework.jmx.export.annotation.ManagedAttribute attr = method.getAnnotation(org.springframework.jmx.export.annotation.ManagedAttribute.class);
                String descr=attr != null ? attr.description() : null;
                exposeManagedAttribute(method, null, descr);
            }
            //or @ManagedOperation
                ...
                exposeManagedOperation(method, description);
            }
            else if (method.isAnnotationPresent(org.springframework.jmx.export.annotation.ManagedOperation.class) ){
            	org.springframework.jmx.export.annotation.ManagedOperation op=method.getAnnotation(org.springframework.jmx.export.annotation.ManagedOperation.class);                
            	String description = op != null? op.description() : "";
                exposeManagedOperation(method, description);
            }
        }
    ...
    private void exposeManagedOperation(Method method, String description) {
        ...
        if(!isAlreadyExposed) {
            ops.add(new MBeanOperationInfo(description, method));
        }
    }
    
    private void exposeManagedAttribute(Method method, String attributeName,String descr){
    ...
    private void findFields() {
        ...
        for(Class<?> clazz=getObject().getClass();clazz != null; clazz=clazz.getSuperclass()) {
            ...
            for(Field field:fields) {
                ...
                boolean expose=attr != null ;

                if(expose) {
                    String fieldName = attributeNameToMethodName((attr!=null && !attr.name().isEmpty())? attr.name():field.getName());
                    String descr=attr != null? attr.description() : null;
                    boolean writable=attr != null? attr.writable() : null;
    ...
    private Attribute getNamedAttribute(String name) {
        ...
        if(name.equals(ABDynaMBean.MBEAN_DESCRITION)) {
            result=new Attribute(ABDynaMBean.MBEAN_DESCRITION, this.description);
        }
        else {
